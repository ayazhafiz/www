...
module HTTP::Vect
  ...
  def get_vector(dim : String = "2D") : Vector2D | Vector3D
    if dim.upcase == "3D"
      vector = get_vector3D
    else
      vector = Vector2D.new i: get_rand, j: get_rand
    end
  end

  # Generates JSON about two 2D vector relationships
  def get_vector_rels(vect_1 : Vector2D | Vector3D,
                      vect_2 : Vector2D | Vector3D) : NamedTuple
  ...
  def process_vector_request(vect_1 : Hash | Nil = nil,
    ...
    if vect_1 && vect_2
      if vect_1.size == vect_2.size
        ...
        {
          error:   3,
          message: api_error path, Error::NOT_DIMENSIONAL,
        }
      end
    else
      {
        error:   2,
        message: api_error path, Error::NOT_PLURAL,
      }
    end
  end
end
